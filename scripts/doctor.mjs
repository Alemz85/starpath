...
function checkCv() {
  if (existsSync(join(projectRoot, 'user/cv.md'))) {
    return { pass: true, label: 'user/cv.md found' };
  }
  return {
    pass: false,
    label: 'user/cv.md not found',
    fix: [
      'Create user/cv.md with your CV in markdown',
      'See config/profile.example.yml for reference',
    ],
  };
}

function checkProfile() {
  if (existsSync(join(projectRoot, 'user', 'profile.yml'))) {
    return { pass: true, label: 'user/profile.yml found' };
  }
  return {
    pass: false,
    label: 'user/profile.yml not found',
    fix: [
      'Run: cp config/profile.example.yml user/profile.yml',
...
function checkPortals() {
  if (existsSync(join(projectRoot, 'user/portals.yml'))) {
    return { pass: true, label: 'user/portals.yml found' };
  }
  return {
    pass: false,
    label: 'user/portals.yml not found',
    fix: [
