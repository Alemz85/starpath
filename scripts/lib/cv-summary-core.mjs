...
//
// user/profile.yml is a per-profile symlink; after a profile switch the new
// target's mtime may be OLDER than a summary generated under the previous
// profile, so the mtime gate alone would silently reuse the wrong summary.
// The generated artifact therefore carries the active slug as an HTML
// comment; `--if-stale` regenerates when the stamp ≠ the current active slug.
// Missing stamp or no profiles/ layout ⇒ always-matching (pre-migration
// artifacts and single-profile repos keep the plain mtime behaviour).

const PROFILE_STAMP_RE = /<!--\s*profile:\s*([a-z0-9][a-z0-9-]*)\s*-->/

/** The stamp line embedded in the generated artifact for a profile slug. */
export function renderProfileStamp(slug) {
  return `<!-- profile: ${slug} -->`
}

/** Extract the profile slug stamped into a generated summary, or null. */
export function extractProfileStamp(text) {
  if (typeof text !== 'string') return null
  const m = text.match(PROFILE_STAMP_RE)
  return m ? m[1] : null
}

/**
 * True when an existing summary's profile stamp is compatible with the
 * current active slug. `activeSlug == null` (no profiles/ layout) or a
 * missing stamp (pre-stamp artifact) both count as matching.
 */
export function profileStampMatches(text, activeSlug) {
  if (!activeSlug) return true
  const stamp = extractProfileStamp(text)
  if (!stamp) return true
  return stamp === activeSlug
}

/**
 * Render the full artifact: generated-file header (plus the active-profile
 * stamp when running under a multi-profile layout), candidate facts from the
 * profile (if any), then the summarized CV.
 */
export function renderCvSummary({ cvText, profileText = '', maxBullets = 4, profileSlug = null }) {
  const parts = [GENERATED_HEADER]
  if (profileSlug) parts.push(renderProfileStamp(profileSlug))
  parts.push('')
