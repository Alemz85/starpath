...
Leer `user/portals.yml` que contiene:
...
**Cada empresa DEBE tener `careers_url` en user/portals.yml.** Si no la tiene, buscarla una vez, guardarla, y usar en futuros scans.
...
1. **Leer configuración**: `user/portals.yml`
...
6. **Filtrar por título** usando `title_filter` de `user/portals.yml`:
...
4. **Guardar la URL encontrada en user/portals.yml** para futuros scans
...
## Mantenimiento del user/portals.yml
