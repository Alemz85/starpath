...
import { CheckCircle2, X as XIcon, AlertTriangle, Trash2 } from 'lucide-react'
 ...
 * both wired to the store's existing `clear` action.
 */
...
function ProcessRow({
  ...
  return (
    <div
      ...
      className={cn(
        'group w-full flex items-start gap-2.5 px-2 py-1.5 rounded-md text-left cursor-pointer transition-colors text-[12px] outline-none',
        focused
          ? 'bg-accent/12 text-text-1'
          : 'hover:bg-bg-elevated text-text-2 focus-visible:bg-bg-elevated',
      )}
    >
      ...
      <div className="flex-1 min-w-0">
        <div className="flex items-center gap-2.5">
          <span className="truncate flex-1 font-medium">{rec.label}</span>
          ...
          {onClear && (
            <button
              ...
              className="shrink-0 -mr-0.5 p-0.5 rounded text-text-4 opacity-0 group-hover:opacity-100 hover:text-danger hover:bg-danger/10 transition-all"
            >
              <XIcon size={11} />
            </button>
          )}
        ...
        <div
          className={cn(
            ...
            rec.status === 'error' ? 'text-danger/80' : 'text-text-4',
          )}
          title={sub.title}
        >
          {sub.text}
