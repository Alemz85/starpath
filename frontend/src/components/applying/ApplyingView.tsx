...
export function ApplyingView() {
  ...
  // Refresh data store on spawn finish (CV PDF generation appends a row, etc.).
  useEffect(() => {
  ...
  const launch = (id: string, label: string, app: ApplicationEntry, modeFile: string) => {
    ...
    // Prefix the prompt with the company+role context so the mode file
    // operates on the right listing. modes/*.md instructions are still
    // followed via the @-import.
      ...
      <div className="flex-1 flex flex-col px-8 pt-8 pb-8 gap-6 overflow-hidden min-h-0">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-body text-text-3">
                {loaded
                  ? `${applications.length} active · ${pipeline.length} pending in inbox`
                  : 'Loading data…'}
              </p>
            </div>
          </div>
        ...
        <div className="flex-1 flex flex-col min-h-0 gap-4">
          ...
          <div className="shrink-0 overflow-x-auto">
            <div className="flex gap-3" style={{ minWidth: STATUS_GROUPS.length * 232 }}>
              {STATUS_GROUPS.map(status => (
                <KanbanColumn
                  ...
                  onViewReport={app => {
                    navigate('reports', `${app.company}|${app.role}`)
                  }}
                />
              ))}
            </div>
          ...
          {/* Activity panel — shared with other spawn surfaces */}
      ...
      <div className="flex items-center justify-between px-3 h-9 border-b border-border-default">
        ...
        <div className="flex items-center gap-1.5">
          <input
            ...
            className="w-64 px-2 py-1 bg-bg-base border border-border-default rounded text-label text-text-1 placeholder:text-text-4 outline-none focus:border-accent/60"
          />
          <button
            onClick={addUrl}
...
function KanbanColumn({ status, items, spawns, onTailorCV, onDraftApp, onPrepInt, onViewReport }: ColumnProps) {
  ...
  return (
    <div className="flex flex-col w-56 shrink-0 rounded-lg bg-bg-panel border border-border-default overflow-hidden">
      ...
      <div className="flex-1 overflow-y-auto p-2 space-y-2 min-h-[120px] max-h-[280px]">
        {items.length === 0 ? (
  ...
  return (
    <div className="p-2.5 rounded-md bg-bg-elevated border border-border-default hover:border-border-strong transition-colors">
      <div className="flex items-start gap-2">
        <CompanyLogo company={app.company} size={22} className="shrink-0 mt-0.5" />
        <div className="min-w-0 flex-1">
          <div className="text-label text-text-1 font-medium truncate">{app.company}</div>
          <div className="text-label text-text-3 truncate">{app.role}</div>
        </div>
      </div>
