// Unit tests for src/lib/offerCompare.ts — the frontend port of the end-of-funnel
// multi-offer comparison engine (scripts/lib/offer-compare.mjs). Mirrors the
// backend suite's coverage — per-factor derivation reusing the canonical scoring
...
// close-call vs clear-pick recommendation branches — plus parity assertions that
// the ported savings band and CF/AF rollups match the documented canonical
// values, so a divergence from the backend math fails here.
//
// Runs under the renderer's zero-dep node:test harness (`npm test`).
...
const baseScores = (): FactorScores => ({ comp: 5, fit: 5, growth: 5, brand: 5, location: 5, risk: 5 })

// ─── parity: ported bands match the documented canonical values ──────────────

test('savingsToBaseScore reproduces every documented band edge', () => {
  // The 10-tier savings-power band from score-bands.mjs / _shared.md.
  assert.equal(savingsToBaseScore(2000), 10)
  assert.equal(savingsToBaseScore(1500), 9)
  assert.equal(savingsToBaseScore(1100), 8)
  assert.equal(savingsToBaseScore(800), 7)
  assert.equal(savingsToBaseScore(500), 6)
  assert.equal(savingsToBaseScore(250), 5)
  assert.equal(savingsToBaseScore(50), 4)
  assert.equal(savingsToBaseScore(-150), 3)
  assert.equal(savingsToBaseScore(-400), 2)
  assert.equal(savingsToBaseScore(-401), 1)
  // monotonic non-decreasing across the whole range
  let prev = -Infinity
  for (let s = -1000; s <= 3000; s += 25) {
    const v = savingsToBaseScore(s)
    assert.ok(v >= prev, `band should be monotonic at savings=${s}`)
    prev = v
  }
})

test('CF/AF rollups apply the 0.3 bottom-range penalty per low dim', () => {
  // No low dims → plain average.
  assert.equal(rollupCurrentFit({ skills_match: 8, ease_of_entry: 7, strategic_fit: 6 } as SixDims), 7)
  // One dim in [1,2] → average minus 0.3.
  assert.equal(rollupAspirationalFit({ growth_mobility: 2, optionality_exit: 8, brand_value: 8 } as SixDims), Number((6 - 0.3).toFixed(2)))
})

test('rollupOverall is CF×0.70 + AF×0.30 with documented context modifiers', () => {
  // Neutral context: no modifiers.
  const neutral = rollupOverall(8, 6, { salary_adj_for_city: 6, work_life_balance: 6 })
  assert.equal(neutral.overall, Number((8 * 0.7 + 6 * 0.3).toFixed(2)))
  assert.equal(neutral.modifiersApplied.length, 0)
  // Salary Adj ≤ 4 → −0.4; WLB ≤ 4 → −0.2 (both stack).
  const dragged = rollupOverall(8, 6, { salary_adj_for_city: 3, work_life_balance: 3 })
  assert.equal(dragged.overall, Number((8 * 0.7 + 6 * 0.3 - 0.4 - 0.2).toFixed(2)))
  // Intern carve-out: salary modifier suppressed, WLB still applies.
  const intern = rollupOverall(8, 6, { salary_adj_for_city: 3, work_life_balance: 3, is_intern: true })
  assert.equal(intern.overall, Number((8 * 0.7 + 6 * 0.3 - 0.2).toFixed(2)))
  assert.ok(intern.modifiersApplied.some((m) => /intern/.test(m.source)))
})
...
test('compFactorFromSavings maps net−baseline through savingsToBaseScore', () => {
  const r = compFactorFromSavings(3000, 1500) // savings 1500 → band 9
  assert.equal(r.monthlySavings, 1500)
  assert.equal(r.score, 9)
  assert.equal(r.score, savingsToBaseScore(1500))
})
...
test('compFactorFromSavings rejects non-finite inputs', () => {
  ...
  assert.throws(() => compFactorFromSavings(3000, undefined as unknown as number), TypeError)
})
...
test('scoreOffer rejects out-of-range and missing factor scores', () => {
  ...
  const missing = { fit: 5, growth: 5, brand: 5, location: 5, risk: 5 } as unknown as FactorScores
...
test('compareOffers flags a close call when the margin is within threshold', () => {
  ...
  assert.ok(res.margin < CLOSE_CALL_THRESHOLD)
...
// ─── buildRecommendation: pure, drives both UI + CLI ─────────────────────────

test('buildRecommendation says "no materially concede" when winner dominates', () => {
  const winner = { label: 'W', total: 8 }
  const runnerUp = { label: 'R', total: 5 }
...
// ─── metadata completeness (the editor depends on it) ────────────────────────

test('every factor has a label and help string', () => {
  for (const f of FACTORS) {
    assert.ok(FACTOR_LABELS[f] && FACTOR_LABELS[f].length > 0)
    assert.ok(FACTOR_HELP[f] && FACTOR_HELP[f].length > 0)
  }
