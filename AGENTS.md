...
- `modes/*`, `AGENTS.md`, `*.mjs` scripts, `templates/*`, `batch/*`, `frontend/*`
...
The Data Contract above is necessary but not sufficient. The system layer leaks user data through *examples and defaults* even when no one set out to write user-specific content. Before committing any change to `modes/*`, `scripts/*`, `templates/*`, `frontend/*`, or `AGENTS.md`, run this checklist:
...
- **Frontend cache:** the Electron app mirrors `data/*` and `reports/**` into a SQLite cache at `{userData}/cache.db` for fast queries. It is fully derived from the Markdown/TSV files and rebuilds on launch via mtime comparison + a chokidar watcher. Markdown/TSV remain canonical; backend modes (Codex) never read or write the cache. See `frontend/ARCHITECTURE.md`.
