...
Validates that all prerequisites are in place: Node.js >= 18, dependencies installed, Playwright chromium, required files (`cv.md`, `user/profile.yml`, `portals.yml`), fonts directory, and auto-creates `data/`, `output/`, `reports/` if missing.
...
Validates that the career-ops setup is internally consistent: `cv.md` exists and is not too short, `user/profile.yml` exists with required fields, no hardcoded metrics in `modes/_shared.md` or `batch/batch-prompt.md`, and `user/article-digest.md` freshness (warns if older than 30 days).
...
**Exit codes:** `0` scan completed, `1` configuration error or no user/portals.yml found.
