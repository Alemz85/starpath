...
6. **Report**: Save as `reports/tier-{N}/{num}-{company}-{date}.md` where `{N}` is derived from Score (≥4.5 → tier-1, 3.5-4.5 → tier-2, <3.5 → tier-3, SKIP → tier-4)
...
user/article-digest.md        →  Proof points for matching
user/profile.yml       →  Candidate identity
user/portals.yml              →  Scanner configuration
